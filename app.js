...
function addPlan(id, date = filters.date) {
  ...
  document
    ...
    .forEach((b) => (b.innerHTML = "<i data-icon=check></i> 已在计划中"));
}
...
function empty(
  ...
  return `<div class="empty"><div class="empty-icon"><i data-icon=arrow-up-right></i></div><h3>${title}</h3><p>${desc}</p><button class="primary" onclick="${handler}">${action} <i data-icon=arrow-right></i></button></div>`;
}
function score(a) {
  return (
...
function openingHours(a) {
  ...
  const preferred = data?.preferred
    ?.map((p) => `${p.start}–${p.end}`)
    .join("、");
  return `<div class="card-hours"><div><span><i data-icon=clock></i> 营业 / 开放时间</span><strong>${esc(title)}</strong></div><p>${esc(note)}</p>${preferred ? `<p class="hours-preferred">建议游玩 ${esc(preferred)} · 仅供游玩安排参考</p>` : ""}</div>`;
}
function card(a) {
  const saved = state.saved.includes(a.id),
    planned = state.plans.includes(a.id);
  return `<article class="card"><div class="card-image"><button class="image-open" data-detail="${a.id}" aria-label="查看${a.title}"><img src="${photo(a.image)}" alt="${a.category}氛围配图" loading="lazy"></button><span class="card-tag">${a.tag}</span><button class="save ${saved ? "saved" : ""}" data-save="${a.id}" aria-label="${saved ? "取消收藏" : "收藏"}${a.title}" aria-pressed="${saved}">${saved ? "<i data-icon=heart-filled></i>" : "<i data-icon=heart></i>"}</button></div><div class="card-body"><div class="card-meta"><span>${a.category}</span><span><i data-icon=clock></i> ${a.hours} 小时</span></div><h3><button data-detail="${a.id}">${a.title}</button></h3><p class="card-description">${a.place}</p>${openingHours(a)}<div class="card-departure"><button class="text-button" onclick="openNavigation('${a.id}')">地图与导航 <i data-icon=compass></i></button><button class="text-button" onclick="editDeparture('${a.id}')">${a.id === "culture-2026" ? "确认场次" : "入口 / 预约"}</button></div><div class="tags">${a.tags.map((t) => `<span class="tag">${t}</span>`).join("")}</div><div class="card-bottom"><span class="price">${a.price === 0 ? "免费" : `¥${a.price}`}<small>${a.price === 0 ? "活动费用" : "/ 人预计"}</small></span><button class="add" data-add="${a.id}">${a.id === "culture-2026" ? "选择具体场次" : planned ? "<i data-icon=check></i> 已加入" : "<i data-icon=plus></i> 加入计划"}</button></div><div class="reason"><i data-icon=sparkles></i> ${reason(a)}</div><div class="source-meta">${a.source ? `<a href="${esc(a.source.url)}" target="_blank" rel="noopener noreferrer">官方来源 <i data-icon=external></i></a>` : ""}<span>${esc(a.kind || "探索提案")}</span></div></div></article>`;
}
function discover() {
  $("#main").innerHTML =
    `<section class="hero"><div class="hero-copy"><span class="pill"><span class="dot"></span> 武汉 · 你的周末灵感站</span><h1>周末不必远行，<br>也能<em>发现新鲜。</em></h1><p>把「不知道去哪」变成「就去这里」。<br>让兴趣、天气和预算，帮你找到刚刚好的小冒险。</p><div class="hero-bottom"><button class="primary" id="explore-button">找点好玩的 <span><i data-icon=arrow-up-right></i></span></button><span class="hero-note">出发的理由，一个就够了。</span></div></div><div class="hero-photo"><div class="weather-card"><small id="weather-heading">武汉 · 周末预报</small><div class="weather-line"><strong id="hero-temperature">—</strong><span class="sun" id="hero-sun"><i data-icon=clock></i></span></div><p id="hero-weather" aria-live="polite">正在获取天气预报…</p><div class="weather-credit"><a href="https://open-meteo.com/" target="_blank" rel="noopener noreferrer">Open-Meteo · CC BY 4.0 <i data-icon=external></i></a><span id="weather-updated"></span><button id="refresh-weather" type="button">刷新预报</button></div></div><div class="photo-label">NATURE IS CALLING. <i data-icon=arrow-up-right></i> <span>氛围配图</span></div></div></section><div class="strip"><strong><i data-icon=sparkles></i> 为你的周末多想一点</strong><span>预算友好</span><span>天气适配</span><span>一个人或一群人</span><span>城市很大，快乐可以很近。</span></div><section id="recommendations"><div class="section-heading"><h2>发现你的周末<small>不赶路，去感受路。</small></h2><button class="text-button" id="random-button">给我一点惊喜 <i data-icon=arrow-up-right></i></button></div><div class="explore-layout"><aside class="filters" aria-label="推荐条件"><div class="filter-title">我的出逃偏好 <button class="reset" id="reset-button">重置</button></div><div class="filters-grid"><div class="filter-group"><label class="filter-label" for="city">探索城市</label><select class="select" id="city"><option>武汉 · 江城慢游</option></select><p class="filter-hint">首期城市，更多地方正在路上</p></div><div class="filter-group"><label class="filter-label" for="budget">人均预算 <output id="budget-value">¥${filters.budget} 以内</output></label><input type="range" id="budget" min="0" max="200" step="5" value="${filters.budget}" aria-label="人均预算"><div class="range-labels"><span>¥0</span><span>¥200</span></div></div><div class="filter-group"><label class="filter-label" for="travel-date">出发日期</label><select class="select" id="travel-date">${WeekendWeather.weekendDates()
      .map(
        (date) =>
...
function updateWeather() {
  ...
  $("#hero-temperature").textContent = row
    ...
    : "—";
  $("#hero-sun").innerHTML = icon(
    forecastLoading && !row ? "clock" : weatherIcon(row?.code),
  );
...
function sourceBlock(a) {
  return `<section class="source-panel"><strong>${esc(a.kind || "旧版记录")}</strong>${a.statusNote ? `<p class="notice">${esc(a.statusNote)}</p>` : ""}${a.source ? `<p><a href="${esc(a.source.url)}" target="_blank" rel="noopener noreferrer">${esc(a.source.name)} <i data-icon=external></i></a></p><small>来源发布：${esc(a.source.publishedAt)} · 整理核对：${esc(a.checkedAt)}${a.endDate ? ` · 有效至 ${esc(a.endDate)}` : ""}</small>${a.extraSource ? `<p><a href="${esc(a.extraSource.url)}" target="_blank" rel="noopener noreferrer">${esc(a.extraSource.name)} <i data-icon=external></i></a></p>` : ""}` : "<p>此条为旧版示例，已停止推荐。</p>"}</section>`;
}
function detail(id) {
  const a = getActivity(id);
  if (!a) return;
  openModal(
    `<img class="detail-image" src="${photo(a.image)}" alt="${a.category}氛围配图"><div class="modal-inner"><span class="eyebrow">${a.category} / WEEKEND IDEA</span><h2>${a.title}</h2><p>${a.desc}</p><div class="detail-facts"><div><small>目的地</small><strong>${a.place}</strong></div><div><small>人均预算估算 · 不含交通</small><strong>${a.price === 0 ? "免费" : `¥${a.price}`} · ${a.hours} 小时</strong></div><div><small>建议出发时段</small><strong>${a.time}</strong></div><div><small>天气 / 同行</small><strong>${a.indoor ? "室内 · 晴雨皆宜" : "户外 · 晴天出发"} / 1–${a.group} 人</strong></div></div>${openingHours(a)}${departureLinks(a)}<h3>一条不赶时间的路线</h3><p>${a.route}</p><h3>出发前的小提醒</h3><p>${a.transport}<br>${a.booking}</p><p class="notice">${a.tip}<br>路线和时长为本产品建议，官方公告不代表实时余票；出发前请核实开放和预约。</p>${sourceBlock(a)}<div class="modal-actions"><button class="secondary" onclick="teamForm('${a.id}')">组队出发 <i data-icon=arrow-up-right></i></button><button class="primary" data-add="${a.id}">${state.plans.includes(a.id) ? "<i data-icon=check></i> 已在计划中" : "<i data-icon=plus></i> 加入我的计划"}</button></div></div>`,
  );
}
function plans() {
...
function showInvite(id, data) {
  ...
  openModal(
    `<div class="modal-inner"><span class="eyebrow">YOUR WEEKEND CREW</span><h2>小队已就绪，邀朋友出发。</h2><div class="team-box"><strong>${esc(data.name)}</strong>${a.title}<br>${esc(data.date)} · ${esc(data.size)} 人计划<br>${esc(data.meeting)}</div><p>邀请链接包含以上安排。对方可保存到自己的计划，成员状态不会自动同步。</p><label class="field">邀请链接（也可手动复制）<input readonly value="${esc(url)}" aria-label="邀请链接"></label><button class="primary full" id="copy-invite">复制小队邀请 <i data-icon=arrow-up-right></i></button></div>`,
  );
  ...
  try {
    ...
    if (type === "invite") {
      ...
      if (
        ...
        throw Error();
      $("#main").innerHTML =
        `<div class="page-title"><span class="shared-badge">来自探索家的周末攻略</span><h1>${esc(data.title)}</h1><p>${esc(data.date)} · 人均实际花费 ¥${esc(data.cost)}</p></div><article class="journal-card"><p>${esc(data.text)}</p><small>${esc(getActivity(data.activity)?.place || "城市探索")}</small></article><p class="notice">用户分享内容，仅供出行参考。请出发前核实相关信息。</p><a href="#discover" class="primary">发现我的周末 <i data-icon=arrow-up-right></i></a>`;
    }
  } catch {
...
document.addEventListener("click", (e) => {
  ...
  if (b.dataset.shareJournal) {
    ...
    if (j) {
      ...
      openModal(
        `<div class="modal-inner"><h2>让你的发现，成为别人的灵感。</h2><p>任何获得链接的人都能阅读这篇攻略。</p><label class="field">攻略分享链接<input readonly value="${esc(url)}" aria-label="攻略分享链接"></label><button class="primary full" id="copy-story">复制攻略链接 <i data-icon=arrow-up-right></i></button></div>`,
      );
